...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):
        weak_batched_inputs, strong_batched_inputs = self.weak_strong_split(batched_inputs)
        ...
        weak_images_norm, strong_images_norm = self.weak_strong_split(images_norm)
        strong_features = self.backbone(strong_images_norm.tensor)
        ...
        weak_gt_instances, strong_gt_instances = self.weak_strong_split(gt_instances)
        
        strong_mask_feats, sem_losses = self.mask_branch(strong_features, strong_gt_instances)

        proposals, proposal_losses = self.proposal_generator(
            strong_images_norm, strong_features, strong_gt_instances, self.controller
        )

        if self.training:
            self._iter += 1
            if self.ema_on:
                weak_features = self.backbone_ema(weak_images_norm.tensor)
                weak_mask_feats, _ = self.mask_branch_ema(weak_features, weak_gt_instances)
                strong_mask_feats_ema = self.mask_branch_ema(strong_features, strong_gt_instances)
                weak_proposals, _ = self.proposal_generator_ema(
                                        weak_images_norm, weak_features, weak_gt_instances, self.controller_ema
                                    )
                mask_losses = self._forward_mask_heads_train(proposals, proposals_ema, mask_feats, mask_feats_ema, gt_instances)
                # update ema model
                if self._iter % 100 == 0: 
                    alpha = 0.99
    ...
    def _forward_mask_heads_train(self, proposals, proposals_ema, mask_feats, mask_feats_ema, gt_instances):
