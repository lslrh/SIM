...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):           
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.training and self.copypaste and len(self.paste_data)>=self.memory_length:
            ...
            if self.ema_on:
                ...
                for i in range(len(batched_inputs)):
                    if 'paste_mask' in gt_instances[i]._fields.keys():
                        ...
                        paste_data['gt_boxes'] = gt_instances[i].gt_boxes.detach().cpu()
                        paste_data['gt_classes'] = gt_instances[i].gt_classes.detach().cpu()
