            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    if self._iter > 10000:
                        ...
                        thr = torch.clamp(thr, min=0.55, max=0.7)
                        ...
                        paste_mask = mask_scores_mean > 0.5
                        paste_weights = ((mask_scores_mean > 0.7) | (mask_scores_mean < 0.3)) 
                        start = 0
                        ...
                        for per_im in gt_instances:
                            ...
                            per_im.paste_weights = paste_weights[start:end].squeeze(1) 
                            score = (mask_scores_mean[start:end].squeeze(1) * per_im.paste_mask).sum(dim=[1,2]) / per_im.paste_mask.sum(dim=[1,2])
                            per_im.score = score
                            start = end
                        # compute mask loss on pasted instances
                        if 'gt_masks' in gt_instances[0]._fields.keys():
                            gt_masks = torch.cat([per_im.gt_masks for per_im in gt_instances])[gt_inds].unsqueeze(1)
                            ...
                            # show_feature_map((gt_masks*weights).detach(), 4)
                            # pdb.set_trace()
                            loss_paste = (mask_focal_loss(mask_scores, gt_masks, weights)) * warmup_factor_2 * 1.0
