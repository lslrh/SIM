...
class CondInst(nn.Module):
    ...
    def weak_strong_split(self, inputs):
        weak_ids = list(range(0, len(inputs), 2))
        strong_ids = list(range(1, len(inputs), 2))
   
        weak_items = []
        strong_items = []
        for i in range(len(weak_ids)):
            weak_items.append(inputs[weak_ids[i]])
            strong_items.append(inputs[strong_ids[i]])
        return weak_items, strong_items

    def forward(self, batched_inputs):
        # w_batched_inputs, s_batched_inputs = self.weak_strong_split(batched_inputs)
        original_images = [x["image"].to(self.device) for x in batched_inputs]
        # normalize images
        images_norm = [self.normalizer(x) for x in original_images]
        w_images_norm, s_images_norm = self.weak_strong_split(images_norm)
        
        images_norm = ImageList.from_tensors(images_norm, self.backbone.size_divisibility)
        w_images_norm = ImageList.from_tensors(w_images_norm, self.backbone.size_divisibility)
        s_images_norm = ImageList.from_tensors(s_images_norm, self.backbone.size_divisibility)

        s_features = self.backbone(s_images_norm.tensor)
        ...
        w_gt_instances, s_gt_instances = self.weak_strong_split(gt_instances)

        s_mask_feats, sem_losses = self.mask_branch(s_features, s_gt_instances)

        s_proposals, proposal_losses = self.proposal_generator(
            s_images_norm, s_features, s_gt_instances, self.controller
        )

        if self.training:
            self._iter += 1
            if self.ema_on:
                w_features = self.backbone_ema(w_images_norm.tensor)
                w_mask_feats, _ = self.mask_branch_ema(w_features, w_gt_instances)
                s_mask_feats_ema = self.mask_branch_ema(s_features, s_gt_instances)
                w_proposals, _ = self.proposal_generator_ema(
                                        w_images_norm, w_features, w_gt_instances, self.controller_ema)
                s_proposals_ema, _ = self.proposal_generator_ema(
                                        s_images_norm, s_features, s_gt_instances, self.controller_ema)

                mask_losses = self._forward_mask_heads_train(s_proposals, s_proposals_ema, w_proposals, s_mask_feats, s_mask_feats_ema, w_mask_feats, w_gt_instances)
            ...
            pred_instances_w_masks = self._forward_mask_heads_test(s_proposals, s_mask_feats)
    ...
    def _forward_mask_heads_train(self, s_proposals, s_proposals_ema, w_proposals, s_mask_feats, s_mask_feats_ema, w_mask_feats, w_gt_instances):
        # prepare the inputs for mask heads
        s_pred_instances = s_proposals["instances"]
        s_pred_instances_ema = s_proposals_ema["instances"]
        w_pred_instances = w_proposals["instances"]
        ...
        if self.max_proposals != -1:
            if self.max_proposals < len(s_pred_instances):
                inds = torch.randperm(len(s_pred_instances), device=s_mask_feats.device).long()
                logger.info("clipping proposals from {} to {}".format(
                    len(s_pred_instances), self.max_proposals
                ))
                s_pred_instances = s_pred_instances[inds[:self.max_proposals]]
        
        elif self.topk_proposals_per_im != -1:
            num_images = len(w_gt_instances)
            s_kept_instances = []
            w_kept_instances = []
            s_kept_instances_ema = []

            for im_id in range(num_images):
                s_instances_per_im = s_pred_instances[s_pred_instances.im_inds == im_id]
                w_instances_per_im = w_pred_instances[w_pred_instances.im_inds == im_id]
                s_instances_per_im_ema = s_pred_instances_ema[s_pred_instances_ema.im_inds == im_id]
                if len(s_instances_per_im) == 0:
                    s_kept_instances.append(s_instances_per_im)
                    w_kept_instances.append(w_instances_per_im)
                    s_kept_instances_ema.append(s_instances_per_im_ema)
                    continue

                unique_gt_inds = s_instances_per_im.gt_inds.unique()
                num_instances_per_gt = max(int(self.topk_proposals_per_im / len(unique_gt_inds)), 1)

                for gt_ind in unique_gt_inds:
                    s_instances_per_gt = s_instances_per_im[s_instances_per_im.gt_inds == gt_ind]
                    w_instances_per_gt = w_instances_per_im[w_instances_per_im.gt_inds == gt_ind]
                    s_instances_per_gt_ema = s_instances_per_im_ema[s_instances_per_im_ema.gt_inds == gt_ind]
                    if len(s_instances_per_gt) > num_instances_per_gt:
                        scores = s_instances_per_gt.logits_pred.sigmoid().max(dim=1)[0]
                        ctrness_pred = s_instances_per_gt.ctrness_pred.sigmoid()
                        inds = (scores * ctrness_pred).topk(k=num_instances_per_gt, dim=0)[1]
                        s_instances_per_gt = s_instances_per_gt[inds]
                        # ema s_gt_instances
                        s_instances_per_gt_ema = s_instances_per_gt_ema[inds]

                        scores = w_instances_per_gt.logits_pred.sigmoid().max(dim=1)[0]
                        ctrness_pred = w_instances_per_gt.ctrness_pred.sigmoid()
                        inds = (scores * ctrness_pred).topk(k=num_instances_per_gt, dim=0)[1]
                        w_instances_per_gt = w_instances_per_gt[inds]

                    s_kept_instances.append(s_instances_per_gt)
                    w_kept_instances.append(w_instances_per_gt)
                    s_kept_instances_ema.append(s_instances_per_gt_ema)

            s_pred_instances = Instances.cat(s_kept_instances)
            w_pred_instances = Instances.cat(w_kept_instances)
            s_pred_instances_ema = Instances.cat(s_kept_instances_ema)

        s_pred_instances.mask_head_params = s_pred_instances.top_feats
        w_pred_instances.mask_head_params = w_pred_instances.top_feats
        s_pred_instances_ema.mask_head_params = s_pred_instances_ema.top_feats
       
        loss_mask = self.mask_head(
            s_mask_feats, s_mask_feats_ema, w_mask_feats, self.mask_branch.out_stride,
            s_pred_instances, s_pred_instances_ema, w_pred_instances, w_gt_instances
        )
    ...
    def _forward_mask_heads_test(self, proposals, mask_feats):
        ...
        pred_instances_w_masks = self.mask_head(
            mask_feats, 0, self.mask_branch.out_stride, pred_instances, 0
        )
