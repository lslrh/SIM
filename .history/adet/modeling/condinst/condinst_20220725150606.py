...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):           
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.training and self.copypaste and len(self.paste_data)>=5:
        ...
        if "instances" in batched_inputs[0]:
            if self.boxinst_enabled:
                ...
                # start.record()
                if self.training and self.copypaste and len(self.paste_data)>=10:
            ...
            if self.ema_on:
                ...
                for i in range(len(batched_inputs)):
                    if 'paste_mask' in gt_instances[i]._fields.keys():
                        paste_data['image'] = original_images.tensor[i]
