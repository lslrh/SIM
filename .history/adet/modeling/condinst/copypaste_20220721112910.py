...
def _copypaste_transform(images_norm, gt_instances, paste_data):
    ...
    for i in range(len(images_norm)):
        ...
        paste_img = torch.from_numpy(paste_data[index]['image']).to(original_img.device)
        paste_gt_bboxes = paste_data[index]['gt_boxes']
        paste_gt_labels = paste_data[index]['gt_classes']
        paste_mask = torch.from_numpy(paste_data[index]['paste_mask']).to(original_img.device)
        ...
        original_img = torch.pad(original_img, ((0, 0), (0, max_W-orig_W), (0, max_H-orig_H)), 'constant')
        original_mask = torch.pad(original_mask, ((0, 0), (0, max_W-orig_W), (0, max_H-orig_H)), 'constant')
        paste_img = torch.pad(paste_img, ((0, 0), (0, max_W-past_W), (0, max_H-past_H)), 'constant')
        paste_mask = torch.pad(paste_mask, ((0, 0), (0, max_W-past_W), (0, max_H-past_H)), 'constant')
