...
class CondInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.ema_on:
            ...
            self.proposal_generator_ema = build_proposal_generator(cfg, self.backbone_ema.output_shape())
            self.mask_head_ema = build_dynamic_mask_head(cfg)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            if self.ema_on:
                ...
                mask_losses = self._forward_mask_heads_train(proposals, mask_feats, gt_instances, ema_mask_feats)
            else:
