...
class DynamicMaskHead(nn.Module):
    ...
    def __call__(self, mask_feats, mask_feats_ema, mask_feat_stride, pred_instances, pred_instances_ema, gt_instances=None):
        if self.training:
            ...
            gt_bitmasks_ = torch.cat([per_im.gt_bitmasks for per_im in gt_instances])
            gt_bitmasks = gt_bitmasks_[gt_inds].unsqueeze(dim=1).to(dtype=mask_feats.dtype)
            ...
            if len(pred_instances) == 0:
                ...
                for per_im in gt_instances:
                    ...
                    if len(per_im)!= len(paste_mask[start:end]):
                        continue
                    pdb.set_trace()
                    per_im.paste_mask = paste_mask[start:end] * gt_bitmasks_.unsqueeze(1)[start:end].squeeze(1)
                    start = end
                else:
