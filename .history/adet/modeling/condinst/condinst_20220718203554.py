...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):           
        ...
        if self.copypaste and len(self.paste_data)>=self.memory_length:
            images_norm, gt_instances = _copypaste_transform(images_norm, gt_instances, self.paste_data)   
            ...
            if self.ema_on:
                ...
                for i in range(len(batched_inputs)):
                    if 'paste_mask' in gt_instances[i]._fields.keys():
                        paste_data['image'] = images_norm.tensor[i].detach().cpu().numpy()
                        paste_data['gt_boxes'] = batched_inputs[i]['instances'].gt_boxes
                        paste_data['gt_classes'] = batched_inputs[i]['instances'].gt_classes
                        paste_data['paste_mask'] = gt_instances[i].squeeze(1).paste_mask.detach().cpu().numpy()
