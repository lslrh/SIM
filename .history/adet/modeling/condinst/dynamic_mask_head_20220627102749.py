            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    for i, x in enumerate(gt_instances):
                        masks = torch.einsum('hwd,npd->nphw', mask_feats_norm[i], self.prototypes[x.gt_classes]).detach()
                    ...
                    pseudo_scores = pseudo_seg.reshape(len(gt_inds), -1)
                    ...
                    warmup_factor_2 = min(self._iter.item() / float(50000), 1.0) if self._iter >10000 else 0 
                    weights = ((pseudo_seg > thr) | (pseudo_seg < (thr-0.1))) * gt_bitmasks
                    loss_pseudo = (mask_focal_loss(mask_scores, pseudo_seg_final, weights)) * warmup_factor_2

                    # update the prototypes
                    gt_classes = [x.gt_classes for x in gt_instances]
                    self.prototype_learning(mask_feats_norm, proto_masks, masks, gt_classes, pred_instances.labels, pred_instances.im_inds)
