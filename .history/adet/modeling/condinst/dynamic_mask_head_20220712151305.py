...
class DynamicMaskHead(nn.Module):
    ...
    def __call__(self, s_mask_feats, w_mask_feats, mask_feat_stride, s_pred_instances, w_pred_instances, gt_instances=None):
        if self.training:
            self._iter += 1

            gt_inds = s_pred_instances.gt_inds
            gt_bitmasks = torch.cat([per_im.gt_bitmasks for per_im in gt_instances])
            gt_bitmasks = gt_bitmasks[gt_inds].unsqueeze(dim=1).to(dtype=s_mask_feats.dtype)

            losses = {}

            if len(s_pred_instances) == 0:
                dummy_loss = s_mask_feats.sum() * 0 + s_pred_instances.mask_head_params.sum() * 0
                ...
                s_mask_logits = self.mask_heads_forward_with_coords(
                    s_mask_feats, mask_feat_stride, s_pred_instances)
                s_mask_scores = s_mask_logits.sigmoid()

                w_mask_logits = self.mask_heads_forward_with_coords(
                    w_mask_feats, mask_feat_stride, w_pred_instances)
                w_mask_scores = w_mask_logits.sigmoid()
                # show_feature_map(mask_feats[0].detach(), 0)
                # show_feature_map(mask_feats[1].detach(), 1)

                if self.boxinst_enabled:
                    # compute feats similarity 
                    s_mask_feats = F.interpolate(
                                                s_mask_feats,
                                                scale_factor=2,
                                                mode="bilinear", align_corners=False)
                    w_mask_feats = F.interpolate(
                                                w_mask_feats,
                                                scale_factor=2,
                                                mode="bilinear", align_corners=False)
                    ...
                    image_color_similarity = image_color_similarity[gt_inds].to(dtype=s_mask_feats.dtype)

                    loss_prj_term = compute_project_term(s_mask_scores, gt_bitmasks)

                    pairwise_losses = compute_pairwise_term(
                        s_mask_logits, self.pairwise_size,
                    ...
                    w_mask_feats = l2_normalize(w_mask_feats).permute(0,2,3,1)
                    s_mask_feats = l2_normalize(s_mask_feats).permute(0,2,3,1)

                    for i, x in enumerate(gt_instances):
                        masks = torch.einsum('hwd,npd->nphw', w_mask_feats[i], self.prototypes[x.gt_classes]).detach()
                        proto_masks_list.append(masks)
                    proto_masks = torch.cat([x for x in proto_masks_list])
                    proto_masks = proto_masks[gt_inds]
                    
                    mask_scores_mean = scatter_mean(w_mask_scores, gt_inds.unsqueeze(1), dim=0)[gt_inds]
                    # compute the threshold for pseudo labels
                    masks = (mask_scores_mean > 0.55) * gt_bitmasks.float()
                    if self._iter > 10000:
                        ...
                        thr = torch.clamp(thr, min=0.5, max=0.6)
                        ...
                        warmup_factor_2 = min(self._iter.item() / float(90000), 1)
                        weights = ((pseudo_seg > thr) | (pseudo_seg < 0.4)) * gt_bitmasks
                        loss_pseudo = (mask_focal_loss(s_mask_scores, pseudo_seg_final.detach(), weights)) * warmup_factor_2 * 0.3
                        losses["loss_pseudo"] = loss_pseudo

                    # update the prototypes
                    gt_classes = [x.gt_classes for x in gt_instances]
                    self.prototype_learning(w_mask_feats, proto_masks, masks, gt_classes, w_pred_instances.labels, w_pred_instances.im_inds)
                    ...
                    mask_losses = dice_coefficient(s_mask_scores, gt_bitmasks)
            ...
            if len(s_pred_instances) > 0:
                ...
                s_pred_instances.pred_global_masks = mask_logits.sigmoid()

            return s_pred_instances
