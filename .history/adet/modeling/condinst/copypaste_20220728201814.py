...
def _copypaste_transform(images_norm, gt_instances, paste_data):
    ...
    for i in range(len(images_norm)):
        ...
        results['paste_weight'] = F.pad(paste_weight, (0, max_H-past_H, 0, max_W-past_W, len(out_mask)-len(paste_weight), 0), 'constant')[:, :orig_W, :orig_H]
...
def annotations_to_instances(results):
    ...
    target.paste_weight = results['paste_weight']
    stride = 4
    start = int(stride // 2)
