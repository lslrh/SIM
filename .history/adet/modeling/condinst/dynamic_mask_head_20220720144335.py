        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    if self._iter>30:
                        pdb.set_trace()
                        indicator = torch.cat([per_im.paste_indicator for per_im in gt_instances])[gt_inds].to(mask_scores.device)
                        loss_paste = (mask_focal_loss(mask_scores, gt_bitmasks, indicator[:,None,None,None])) * warmup_factor_2 * 0.5
