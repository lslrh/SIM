...
def _copypaste_transform(images_norm, gt_instances, paste_data):
    ...
    for i in range(len(images_norm)):
        ...
        paste_weight = paste_weight[index]['paste_weight'].to(original_img.device)
        ...
        pdb.set_trace()
        results['paste_weights'] = F.pad(paste_mask, (0, max_H-past_H, 0, max_W-past_W, len(out_mask)-len(paste_weight), 0), 'constant')[:, :orig_W, :orig_H]
