...
def _copypaste_transform(images_norm, gt_instances, paste_data):
    ...
    for i in range(len(images_norm)):
        ...
        index = torch.LongTensor(random.randint(0, len(paste_data))).to(paste_data.device)
        ...
        n_select = min(n_select, 5)

        paste_indexs = torch.LongTensor( random.choices(range(0, n_objects), weights=paste_data[index]['score']**3, k=n_select) ).to(paste_gt_bboxes.device)
