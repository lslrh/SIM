        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    pseudo_seg_final = ( pseudo_seg > self.pseudo_thresh) * gt_bitmasks.float()
                    ...
                    n_pos = torch.sum(masks, dim=(1,2,3)).long()
                    mask_scores = mask_scores.reshape(len(gt_inds), -1)
                    mask_scores_rank = torch.sort(mask_scores, descending=True)[1]
                    pdb.set_trace()
                    thr = torch.index_select(mask_scores_rank, dim=1, index=n_pos)
                    pdb.set_trace()
                    thr = mask_scores_rank[n_pos]
