...
def _copypaste_transform(images_norm, gt_instances, paste_data):
    for i in range(len(images_norm)):
        # get data
        original_img = images_norm[i].cpu().numpy()
        # original_gt_bboxes = original_data['gt_bboxes']
        original_gt_labels = gt_instances[i].gt_classes
        original_mask = add_bitmasks_from_boxes(gt_instances[i], original_img.tensor.size(-2),\
             original_img.tensor.size(-1))
        pdb.set_trace()
        ...
        original_img = torch.from_numpy(original_img[:,:orig_W,:orig_H]).to(images_norm.device)
...
def add_bitmasks_from_boxes(gt_instance, im_h, im_w):
    ...
    return gt_instance.gt_bitmasks_full  
