...
from adet.utils.contrast import l2_normalize
...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.prototypes = nn.Parameter(torch.zeros(self.num_classes, self.num_proto, self.in_channels), requires_grad=True)
    ...
    def prototype_learning(self, mask_feat, pseudo_seg, gt_classes):
        gt_classes_unique = torch.unique(gt_classes)
        for k in range(len(gt_classes_unique)):
        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    # compute pseudo loss
                    proto_masks_list = []
                    self.prototypes.data.copy_(l2_normalize(self.prototypes)).permute(0,2,3,1)
                    mask_feats_norm = l2_normalize(mask_feats_)
                    _c = rearrange(mask_feats_norm, "b c h w -> (b h w) c")
                    ...
                    pseudo_seg = proto_masks[gt_inds]
                    pseudo_seg = (torch.amax(pseudo_seg, dim=1).unsqueeze(1) > self.pseudo_thresh) * gt_bitmasks.float()
                    loss_pseudo = dice_coefficient(mask_scores, pseudo_seg) * warmup_factor

                    # show_feature_map(pseudo_seg.detach(), 0)
                    pdb.set_trace()

                    # update the prototypes
                    gt_classes = torch.cat([x.gt_classes for x in gt_instances])
                    self.prototype_learning(mask_feats, proto_masks, gt_classes)
