            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    if self._iter > 10000:
                        ...
                        mask_scores_mean_full = F.interpolate(
                                                        mask_scores_mean,
                                                        scale_factor=4,
                                                        mode="bilinear", align_corners=False)

                        paste_mask = mask_scores_mean_full > 0.5
                        ...
                        for per_im in gt_instances:
                            ...
                            score = (mask_scores_mean_full[start:end].squeeze(1) * per_im.paste_mask).sum(dim=[1,2]) \
                                / (per_im.paste_mask.sum(dim=[1,2])+1)
                            per_im.score = score
                            start = end
                        # compute mask loss on pasted instances
                        if 'gt_weight' in gt_instances[0]._fields.keys():
                            gt_masks = torch.cat([per_im.gt_bitmasks for per_im in gt_instances])[gt_inds].unsqueeze(1)
                            pdb.set_trace()
                            weights = torch.cat([per_im.gt_weight for per_im in gt_instances])[gt_inds]
