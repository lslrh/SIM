...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        weak_original_images = [x["image"].to(self.device) for x in weak_batched_inputs]
        strong_original_images = [x["image"].to(self.device) for x in strong_batched_inputs]
        # normalize images
        weak_images_norm = [self.normalizer(x) for x in weak_original_images]
        weak_images_norm = ImageList.from_tensors(weak_images_norm, self.backbone.size_divisibility)

        strong_images_norm = [self.normalizer(x) for x in strong_original_images]
        strong_images_norm = ImageList.from_tensors(strong_images_norm, self.backbone.size_divisibility)

        strong_features = self.backbone(strong_images_norm.tensor)
        features = self.backbone(weak_images_norm.tensor)
        ...
        if self.training:
            self._iter += 1
            if self.ema_on:
                ema_features = self.backbone(weak_images_norm.tensor)
