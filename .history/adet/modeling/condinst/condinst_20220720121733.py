...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):           
        ...
        if self.copypaste and len(self.paste_data)>=self.memory_length:
            images_norm, gt_instances = _copypaste_transform(original_images, gt_instances, self.paste_data)
            ...
            if self.ema_on:
                ...
                for i in range(len(batched_inputs)):
                    if 'paste_mask' in gt_instances[i]._fields.keys():
                        paste_data['image'] = original_images.tensor[i].detach().cpu().numpy()
                        paste_data['gt_boxes'] = batched_inputs[i]['instances'].gt_boxes
                        paste_data['gt_classes'] = batched_inputs[i]['instances'].gt_classes
                        paste_data['paste_mask'] = gt_instances[i].paste_mask.detach().cpu().numpy()
    ...
    def add_image_color_similarity(self, instances, images, image_masks, im_h, im_w):
        stride = self.mask_out_stride
        start = int(stride // 2)
