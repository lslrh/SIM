...
def _copypaste_transform(images_norm, gt_instances, paste_data):
    ...
    for i in range(len(images_norm)):
        ...
        original_mask = gt_instances[i].gt_masks_full.numpy()
        ...
        results['gt_labels'] = torch.tensor(out_labels, dtype=torch.int64).to(images_norm.device)
        results['gt_masks'] = torch.tensor(out_mask[:, :orig_W, :orig_H]).to(images_norm.device)
