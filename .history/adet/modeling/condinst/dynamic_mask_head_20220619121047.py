        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    # compute pseudo loss
                    pred_labels = torch.cat([x.gt_classes for x in gt_instances])
                    pred_masks_list = []
                    self.prototypes.data.copy_(l2_normalize(self.prototypes))
                    mask_feats_norm = l2_normalize(mask_feats).permute(0,2,3,1)
                    ...
                    pred_masks = torch.cat([x for x in mask_feat_similarity_list])
                    pred_masks = pred_masks[gt_inds]
